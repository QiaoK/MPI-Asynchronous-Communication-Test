...
int save_all_timing(int rank, int procs, int ntimes, int comm_size, Timer *timers) {
    ...
    fclose(stream);
    sprintf(filename,"total_times_%d.csv",comm_size);
    stream = fopen(filename,"w");
    for ( i = 0; i < procs; ++i ){
        fprintf(stream, "%d",i);
        for ( j = 0; j < ntimes; ++j ) {
    ...
    fclose(stream);
    sprintf(filename,"post_request_time_%d.csv",comm_size);
    stream = fopen(filename,"w");
    for ( i = 0; i < procs; ++i ){
        fprintf(stream, "%d",i);
        for ( j = 0; j < ntimes; ++j ) {
    ...
    fclose(stream);
    sprintf(filename,"barrier_time_%d.csv",comm_size);
    stream = fopen(filename,"w");
    for ( i = 0; i < procs; ++i ){
        fprintf(stream, "%d",i);
        for ( j = 0; j < ntimes; ++j ) {
...
int main(int argc, char **argv){
    ...
    for ( i = 0; i < iter; ++i ){
        if (method == 0 || method == 1){
            ...
            MPI_Reduce((double*)(&timer1), (double*)(&max_timer1), sizeof(Timer), MPI_BYTE, MPI_MAX, 0, MPI_COMM_WORLD);
            if (rank == 0){
